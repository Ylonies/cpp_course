...
template <typename T, typename Allocator>
const Allocator& Deque<T, Allocator>::get_allocator() const {
  return alloc_;
}
...
template <typename T, typename Allocator>
void Deque<T, Allocator>::delete_map() {
  for (T* bucket : map_) {
...
Deque<T, Allocator>& Deque<T, Allocator>::operator=(const Deque& other) {
  ...
  try {
    for (auto elem = other.cbegin(); elem != other.cend(); ++elem) {
      size_t index = elem - other.cbegin();
...
void Deque<T, Allocator>::pop_back() {
  ...
  if (end_ % kBucketSize == 1) {
    alloc_traits::deallocate(alloc_, map_[end_ / kBucketSize], kBucketSize);
...
void Deque<T, Allocator>::pop_front() {
  ...
  if (start_ % kBucketSize == kBucketSize - 1) {
    alloc_traits::deallocate(alloc_, map_[start_ / kBucketSize], kBucketSize);
